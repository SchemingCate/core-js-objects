...
function mergeObjects(objects) {
  ...
  const arrOfEntries = [];
  objects.forEach((obj) => {
    arrOfEntries.push(Object.entries(obj));
  });
...
function makeWord(lettersObject) {
  const wordLengthArr = Object.values(lettersObject)
    .flat()
    .sort((a, b) => a - b);
  const wordLength = wordLengthArr[wordLengthArr.length - 1];
...
function sortCitiesArray(arr) {
  return arr.sort((obj1, obj2) => {
    if (obj1.country.charCodeAt(0) === obj2.country.charCodeAt(0)) {
      return obj1.city.charCodeAt(0) - obj2.city.charCodeAt(0);
    }
    return obj1.country.charCodeAt(0) - obj2.country.charCodeAt(0);
  });
}
